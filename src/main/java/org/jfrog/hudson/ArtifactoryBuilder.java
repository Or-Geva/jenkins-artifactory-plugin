...
public class ArtifactoryBuilder extends GlobalConfiguration {
    ...
    public static final class DescriptorImpl extends Descriptor<GlobalConfiguration> {
        ...
        public FormValidation doTestConnection(
            ...
            ManagerBase[] managerArray = {artifactoryManager, distributionManager};
            try {
                if (!bypassProxy && Jenkins.get().proxy != null) {
                    Stream.of(artifactoryManager, distributionManager).forEach(manager -> manager.setProxyConfiguration(createProxyConfiguration()));
                }

                if (StringUtils.isNotBlank(timeout)) {
                    Stream.of(artifactoryManager, distributionManager).forEach(manager -> manager.setConnectionTimeout(Integer.parseInt(timeout)));
                }
        ...
        private String createTestConnectionMessage(ManagerBase manager, String targetUrl, String serverName) {
            try {
                return String.format("Found %s version: %s on %s", serverName, manager.getVersion().toString(), targetUrl);
